...
Display::Display() {
  ...
  // sets the width, height, bpp, pitch, frontbuffer, and backbuffer from the
  // info block
  width_ = info->width;
  height_ = info->height;
  bpp_ = info->bpp;
  ...
  backbuffer_ =
      reinterpret_cast<uint32_t>(malloc(width_ * height_ * (bpp_ / 8)));
}

void Display::clear_screen(uint32_t color) {
  for (int i = 0; i < width_ * height_; ++i) {
    uint32_t *loc =
        reinterpret_cast<uint32_t *>(backbuffer_ + (i * (bpp_ / 8)));
    *loc = color;
  }
...
void Display::draw_pixel(uint32_t x, uint32_t y, uint32_t color) {
  ...
  uint32_t *loc =
      reinterpret_cast<uint32_t *>(y * pitch_ + (x * (bpp_ / 8)) + backbuffer_);
  *loc = color;
}
...
void Display::swap_buffers() {
  ...
  memcpy(reinterpret_cast<uint32_t *>(frontbuffer_), // to
...
void Display::draw_char(const char c, const uint32_t color) {
  ...
  for (int row = 0; row < 8; ++row) {
    ...
    for (int pixel = 0; pixel < 8; ++pixel) {
      uint32_t draw_col = ((chr_row & (1 << pixel)) >> pixel) * color;
      if (draw_col)
        draw_pixel(pixel + x_add, row + y_add, draw_col);
    }
  }
